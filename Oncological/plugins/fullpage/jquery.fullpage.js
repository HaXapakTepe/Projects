  ...
  $.fn.fullpage = function (options) {
    ...
    $('.fp-section')
      ...
      .done(function () {
        ...
        if (
          activeSlide.length &&
          ($('.fp-section.active').index('.fp-section') !== 0 ||
            ($('.fp-section.active').index('.fp-section') === 0 && activeSlide.index() !== 0))
        ) {
        ...
        if (options.navigation) {
          ...
          nav
            .find('li')
            .eq($('.fp-section.active').index('.fp-section'))
            .find('a')
            .addClass('active')
        }
        ...
        if (destiny.length) {
          ...
          if (!options.animateAnchor && section.length) {
            ...
            $.isFunction(options.afterLoad) &&
              options.afterLoad.call(section, destiny, section.index('.fp-section') + 1)
    ...
    function scrollPage(element, callback, isMovementUp) {
      ...
      if (
        ...
        (options.scrollBar && $(window).scrollTop() === v.dtop)
      ) {
        return
      }
      ...
      $.isFunction(options.onLeave) &&
        !v.localIsResizing &&
        options.onLeave.call(v.activeSection, v.leavingSection, v.sectionIndex + 1, v.yMovement)
    ...
    function hashChangeHandler() {
      if (!isScrolling) {
        ...
        if (section.length) {
          ...
          if (
            (section && section !== lastScrolledDestiny && !isFirstSlideMove) ||
            isFirstScrollMove ||
            (!slideMoving && lastScrolledSlide != slide)
          ) {
            scrollPageAndSlide(section, slide)
          }
        }
    ...
    function landscapeScroll(slides, destiny) {
      ...
      if (options.onSlideLeave) {
        ...
        if (!localIsResizing && xMovement !== 'none') {
          $.isFunction(options.onSlideLeave) &&
      ...
      if (options.css3) {
        ...
        addAnimation(slides.find('.fp-slidesContainer'), options.scrollingSpeed > 0).css(
    ...
    function addTableClass(element) {
      element
        .addClass('fp-table')
        .wrapInner('<div class="fp-tableCell" style="height:' + getTableHeight(element) + 'px;" />')
    }
    ...
    function addTouchHandler() {
      if (isTouchDevice || isTouch) {
        ...
        $(document)
          ...
          .on('touchstart ' + MSPointer.down, touchStartHandler)
        $(document)
          .off('touchmove ' + MSPointer.move)
          .on('touchmove ' + MSPointer.move, touchMoveHandler)
      }
    }
