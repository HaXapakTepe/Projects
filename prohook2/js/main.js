$(document).ready(function () {
  ...
  if (document.querySelector('.shuffle-me')) {
    if (innerWidth > 992) {
      const shuffleMe = document.querySelectorAll('.shuffle-me')
      shuffleMe.forEach((item) => {
        const cardItemImage = item.querySelectorAll('.card__item-image')
        console.log(item)
        if (cardItemImage.length >= 2) {
          $(item).shuffleImages({
            trigger: 'imageMouseMove',
            // hoverTrigger: 500,
          })
  ...
  targetMap.forEach((elem) => {
    elem.addEventListener('mouseover', function (e) {
      if (elem.classList.contains('variety__item--active')) {
      } else {
        e.preventDefault()
        ...
        function (result) {
          ...
          mapButtons.forEach((elem) => {
            MyBalloonLayout = ymaps.templateLayoutFactory.createClass(
                ...
                '</div>',
              {
                build: function () {
                  ...
                  this._$element.on('click', $.proxy(this.onCloseClick, this))
                },
            ...
            var placemark = new ymaps.Placemark(
              [elem.dataset.latitude, elem.dataset.longitude],
              {},
              {
