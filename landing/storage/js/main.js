...
var targetMap = document.querySelectorAll('[data-target]'),
  map = document.querySelectorAll('.storage__box');
...
targetMap.forEach(elem => {
  elem.addEventListener('click', function (e) {
    ...
    map.forEach(elem => {
      elem.classList.remove('storage__box--opacity', 'storage__box--active');
    });
    targetMap.forEach(elem => {
      elem.classList.remove('storage__text--active');
    });
    this.classList.add('storage__text--active');
    var cat = document.querySelector('[data-info="' + target + '"]');
    cat.classList.add('storage__box--active');
    setTimeout(() => {
      cat.classList.add('storage__box--opacity');
    }, 400);
  });
});
