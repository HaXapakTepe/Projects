window.addEventListener('DOMContentLoaded', () => {
  ...
  const submitFooterForm = (e) => {
    ...
    const formControls = [nameInput, telInput, emailInput]
  ...
  const submitModalForm = (e) => {
    e.preventDefault()
    ;[modalNameInputEl, modalEmailInputEl, modalTelephoneInputEl].forEach((input) => {
      if (!checkInputValidity(input)) {
        input.classList.add('input--invalid')
        return
      } else {
    ...
    if (modalNameInputEl.value.length < 2 || modalNameInputEl.value.length > 30) {
  ...
  const submitMiddleForm = (e) => {
    e.preventDefault()
    ;[middleNameInputEl, middleEmailInputEl, middleTelephoneInputEl].forEach((input) => {
      if (!checkInputValidity(input)) {
        input.classList.add('input--invalid')
        return
      } else {
    ...
    if (middleNameInputEl.value.length < 2 || middleNameInputEl.value.length > 30) {
