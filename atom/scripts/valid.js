window.addEventListener('DOMContentLoaded', () => {
  ...
  const contactsFormEl = document.querySelector('#contacts-form')
  ...
  const contactsNameInputEl = contactsFormEl?.querySelector('#contacts-name-input')
  const contactsEmailInputEl = contactsFormEl?.querySelector('#contacts-email-input')
  const contactstextareaInputEl = contactsFormEl?.querySelector('#contacts-textarea-input')
  ...
  const submitContactsForm = (e) => {
    e.preventDefault()
    ;[contactsNameInputEl, contactstextareaInputEl, contactsEmailInputEl].forEach((input) => {
      if (!checkInputValidity(input)) {
        input.classList.add('input--invalid')
        return
      } else {
    ...
    if (contactsNameInputEl.value.length < 2 || contactsNameInputEl.value.length > 30) {
      contactsNameInputEl.classList.add('input--invalid')
      return
    } else {
      contactsNameInputEl.classList.remove('input--invalid')
    }

    if (contactstextareaInputEl.value.length < 5) {
      contactstextareaInputEl.classList.add('input--invalid')
      return
    } else {
      contactstextareaInputEl.classList.remove('input--invalid')
    }

    if (!emailRegex.test(contactsEmailInputEl.value)) {
      contactsEmailInputEl.classList.add('input--invalid')
      return
    } else {
      contactsEmailInputEl.classList.remove('input--invalid')
    }
    contactsFormEl.submit()
  }
  ...
  contactsFormEl?.addEventListener('submit', submitContactsForm)
